...
namespace FIT_PONG.Services.BL
{
    public class Evidentor
    {
        ...
        public List<string> GetZadnjeUtakmice(int brojUtakmica = 5)
        {
            ...
            List<Utakmica> zadnje = db.Utakmice.AsNoTracking().Include(c => c.UcescaNaUtakmici).Include(c => c.Runda).ThenInclude(c => c.Bracket).ThenInclude(c => c.Takmicenje)
                .Where(c => c.IsEvidentirana).OrderByDescending(c => c.DatumVrijeme).Take(brojUtakmica).ToList();
